<template>
  <div>
    <el-card class="cardStyle" v-loading="loading">
      ...
      <el-button @click="batchMethod" type="info" size="small" :disabled="dataTable.length <= 0"
                 style="margin-bottom: 15px" v-if="userInfo.level !== 2">批量编辑
      </el-button>
      ...
      <el-button @click="exportMethod" type="success" :disabled="dataTable.length <= 0" size="small"
                 style="margin-bottom: 15px">导出
      </el-button>
      <el-button @click="sendTranscript" type="success" size="small" style="margin-bottom: 15px"
                 v-if="userInfo.level === 0">发送成绩单
      </el-button>
      <!--过滤框-->
            ...
            <el-row v-else>
              <el-col :span="8">
                <el-form-item label="专业：" prop="profession">
                  <el-select v-model="form.professionObj" style="width: 90%" @change="professionChange"
                             value-key="profession">
                    <el-option v-for="item in classArr" :key="item.profession" :label="item.profession"
...
export default {
      ...
      dataColumns: [
        {
          ...
          minWidth: '60',
          render: (h, params) => {
            if (this.batch || this.showInput === params.row.id) {
              return h('el-input', {
                props: {
  ...
  methods: {
    sendTranscript() {
      this.axiosHelper.post('/api/sms/user/admin/sendTranscript').then(res => {
        this.$message.success({
          message: '发送成功，请通知学生注意查收'
        })
      }).catch(error => {
        this.$message.success({
          message: '发送失败，请稍后重试'
    ...
    editMethod(data) {
      this.showInput = this.showInput === data.id ? '' : data.id;
      this.batch = false;
    },
    batchMethod() {
