                ...
                <el-scrollbar style="height:100%;" wrapStyle="overflow-x:hidden;padding-right:12px;" viewStyle="">
                  <el-dropdown-item v-for="cookies in dealCookie" :key="cookies.username"
                    ...
                    <div>
                      ...
                      <!--删除账号信息按钮-->
                      <div title="删除账号信息" class="deleteCook el-icon-close"
                           @click.stop="deleteCookies(cookies)"></div>
                    </div>
                  </el-dropdown-item>
                </el-scrollbar>
              </el-dropdown-menu>
    ...
    checkLogin(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.loadingBtn = '登陆中 ...';
          this.loginBtn = true;
          this.unEncryptPassword = this.form.password;
          ...
          this.$nextTick(() => {
            _this.axiosHelper.get('/api/sms/user/login', {params: this.form}).then(
              response => {
                ...
                data.password = this.unEncryptPassword;
